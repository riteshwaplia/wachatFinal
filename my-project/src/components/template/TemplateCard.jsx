...
const TemplateCard = ({
  ...
  const handleSubmitToMeta = async (template) => {
    try {
      const res = await api.post(`/templates/${template._id}/submit-to-meta`, {
        businessProfileId,
      });
      console.log("res", res);
    } catch (error) {
        ...
        <div className="bg-white rounded-lg shadow-sm w-full max-w-xs mx-auto overflow-hidden">
          ...
          <div className="bg-[#00a884] p-2 flex items-center">
            ...
            <div className="ml-2 flex-1">
              ...
              <div className="text-white text-xxs opacity-80">
                WhatsApp Business
              </div>
            </div>
          ...
          {/* Message content */}
          <div className="p-1 dark:bg-dark-surface dark:text-dark-text-primary text-[#111b21]">
              ...
              switch (component.type) {
                      ...
                      {component.format === "IMAGE" ? (
                        <div className="relative">
                          <img
                            ...
                            onError={(e) => {
                              e.target.src =
                                "https://placehold.co/300x120?text=Image+Not+Found";
                            }}
                          />
                ...
                case "BODY":
                  return (
                    <div
                      key={index}
                      className="text-sm mb-3 whitespace-pre-line leading-tight"
                    >
                ...
                case "FOOTER":
                  return (
                    <div
                      key={index}
                      className="text-xxs text-[#667781] mt-2 mb-3"
                    >
...
<div
  key={index}
  className="carousel mt-4 flex space-x-4 overflow-x-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100"
>
  {component.cards?.map((card, cardIdx) => (
    <div
      key={cardIdx}
      className="min-w-[250px] max-w-[250px] border rounded-lg shadow-sm bg-white dark:bg-dark-surface overflow-hidden flex-shrink-0"
    >
      {card.components?.map((c, cIdx) => {
        if (c.type === "HEADER" && c.format === "IMAGE") {
          return (
            <img
              ...
              onError={(e) =>
                (e.target.src =
                  "https://placehold.co/300x150?text=Image+Not+Found")
              }
            />
          );
        }
        ...
        if (c.type === "BODY") {
          return (
            <div
              key={cIdx}
              className="p-3 text-sm text-[#111b21] dark:text-dark-text-primary"
            >
          ...
          return (
            <div key={cIdx} className="p-3 space-y-2">
              {c.buttons?.map((btn, bIdx) => (
                <button
                  key={bIdx}
                  className="w-full px-3 py-2 rounded-lg text-sm bg-[#f0f2f5] text-[#008069] border border-[#008069] hover:bg-[#e6f2ef]"
                >
                  {btn.text}
                </button>
              ))}
