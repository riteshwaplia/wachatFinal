...
const ContactPage = () => {
  ...
  const handleBlackListRowSelect = (id) => {
    if (blcakListSelectedrows.includes(id)) {
      setBlackListSelectedrows(
        blcakListSelectedrows.filter((rowid) => rowid !== id)
      );
    } else {
  ...
  const filteredBlacklistedContacts = blacklistedContacts.filter(
    (contact) =>
      contact.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      contact.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      contact.mobileNumber?.includes(searchTerm)
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (activeTab === "contactList") {
        contactRes = await api.get(
          `/projects/${projectId}/contacts/contactList`,
          {
            ...config,
            params,
          }
      ...
      if (activeTab === "blockList") {
        blacklistRes = await api.get(
          `/projects/${projectId}/contacts/blackList`,
          {
            params,
          }
        );
      ...
      if (
        (activeTab === "contactList" &&
          (!contactRes?.data.data || contactRes.data.data.length === 0)) ||
        (activeTab === "blockList" &&
          (!blacklistRes?.data.data || blacklistRes.data.data.length === 0))
      ) {
        setMessage("No contacts found. Add your first contact below!");
        setMessageType("info");
      }
  ...
  const handleDeleteContact = async () => {
    ...
    try {
      const res = await api.delete(
        `/projects/${projectId}/contacts/deleteContact/${selectedContactId}`
      );
  ...
  const handleBlockUnblockContact = async (contact_id, isBlocking) => {
    ...
    try {
      ...
      setMessage(
        res.data.message ||
          `Contact ${isBlocking ? "blocked" : "unblocked"} successfully!`
      );
      setMessageType("success");
      fetchData();
      ...
      console.error(
        `Error ${isBlocking ? "blocking" : "unblocking"} contact:`,
        error
      );
  ...
  const handleUploadContacts = async (e) => {
    ...
    try {
      const res = await api.post(
        `/projects/${projectId}/contacts/uploadContact`,
        formData,
        {
  ...
  if (!user) {
    return (
      <ErrorMessage fullPage message="Please log in to manage your contacts." />
    );
  }
    ...
    <div className="space-y-6 p-6">
      ...
      {blockModel && (
        ...
        >
          ...
          <div className="flex justify-end space-x-3">
            ...
            <Button
              variant="primary"
              onClick={() => handleBlockUnblockContact(null, true)}
            >
              {t("confirm")}
            </Button>
          </div>
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-gray-600">{t("manageProjectContacts")}</p>
          </div>
        ...
        <div className="border-b border-gray-200">
          <nav className="-mb-px flex space-x-1 md:space-x-8">
            ...
            >
              <Ban size={18} />
              <span>
                {t("blockList")} ({blacklistedContacts.length})
              </span>
            </button>
            <button
              onClick={() => setActiveTab("uploadCSV")}
      ...
      <div className="mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 w-full lg:w-auto">
          <Button className="" onClick={handleOpenAddModal}>
            {t("addNewField")}
          </Button>
          <Button className="" onClick={handleOpenShowModal}>
            {t("showField")}
          </Button>
              ...
              >
                ...
                <FiChevronDown
                  className={`transition-transform ${
                    isbulkOption ? "transform rotate-180" : ""
                  }`}
                />
              </Button>
              ...
              {isbulkOption && (
                <div className="absolute right-0 dark:bg-dark-surface mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10">
                  ...
                  >
                    {activeTab === "contactList" ? (
                      <>
                        <FiArchive className="mr-2" /> {t("blockContacts")}{" "}
                      </>
                    ) : (
                      <>
                        <FiUnlock className="mr-2" /> {t("unBlockContacts")}
                      </>
                    )}
                  </button>
                ...
                {isLoading ? (
                  ...
                  <div>
                    {isLoading ? (
                      ...
                      <table
                        onClick={() => setIsbulkOption(false)}
                        className="min-w-full dark:divide-dark-border  divide-gray-200"
                      >
                            ...
                            >
                              <td>
                                <input
                                  ...
                                  className="w-14"
                                  onChange={() => handleRowSelect(contact._id)}
                                />
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                  ...
                                  <div className="ml-4">
                                    <div className="text-sm font-medium dark:text-dark-text-primary text-gray-900">
                                      {contact.name}
                                    </div>
                              ...
                              {/* <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                  ...
                                  <div className="ml-4">
                                    <button
                                    onClick={() => navigate(`/project/${projectId}/contacts/${contact._id}`)}
                                      className="text-sm font-medium dark:text-dark-text-primary text-gray-900 hover:underline"
                                    >
                                      chat on whatsapp
                                    </button>
                                  </div>
                  ...
                  <thead className="bg-gray-50  ">
                    <tr>
                      <th className="w-14">
                        <input
                          type="checkbox"
                          onChange={handleBlackListSelectAll}
                        />
                      </th>
                      <th
                        scope="col"
                      ...
                      >
                        <td>
                          <input
                            type="checkbox"
                            className="w-14"
                            checked={blcakListSelectedrows.includes(
                              contact._id
                            )}
                            onChange={(e) =>
                              handleBlackListRowSelect(contact._id)
                            }
                          />
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex justify-end space-x-2">
                            <Button
                              onClick={() =>
                                handleBlockUnblockContact(contact._id, false)
                              }
                              variant="ghost"
                              size="sm"
                              className="text-gray-600 hover:text-red-600"
          ...
          <div className="lg:p-8">
            <div className="w-full mx-auto">
                      ...
                      <div className="space-y-1 text-center">
                        <div className=" text-sm text-gray-600 justify-center">
                          <div className="text-center mb-8 mt-3">
                            ...
                            <h2 className="text-2xl font-bold text-gray-900 mb-2 dark:text-dark-text-primary">
                              {t("importContacts")}
                            </h2>
                            <p className="text-gray-600">
                              {t(
                                "uploadCSVOrExcelFileToImportMultipleContactsAtOnce"
                              )}
                            </p>
                          </div>
                          <label
                          ...
                          >
                            {!file && (
                              <div className="border  px-2 py-2 bg-blue-100 dark:bg-dark-surface dark:border-dark-border flex items-center justify-center mb-4 gap-2 rounded inline-block">
                                {t("uploadAFile")}
              ...
              <div className="bg-white dark:bg-dark-surface dark:border-dark-border rounded-lg shadow p-6 border border-gray-200">
                <h3 className="text-lg dark:text-dark-text-primary font-medium text-gray-900 mb-4">
                  {t("fileFormatRequirements")}
                </h3>
                <div className="prose prose-sm text-gray-500 mb-6">
                  <p>{t("yourImportFileShouldFollowThisFormat")}</p>
                  <ul className="list-disc pl-5 space-y-1">
                    <li>
                      {t("firstRowMustContainHeadersNameEmailPhoneGroups")}
                    </li>
      ...
      {(activeTab === "contactList" || activeTab === "blockList") &&
        pagination.total > 0 && (
                  ...
                  {Array.from(
                    ...
                    (_, i) => {
                      ...
                      if (pagination.totalPages <= maxVisiblePages) {
                        ...
                        pagination.currentPage <= Math.ceil(maxVisiblePages / 2)
                      ) {
                        pageNum = i + 1;
                      } else if (
                        pagination.currentPage >=
                        pagination.totalPages - Math.floor(maxVisiblePages / 2)
                      ) {
                        pageNum =
                          pagination.totalPages - maxVisiblePages + 1 + i;
                      } else {
                        pageNum =
                          pagination.currentPage -
